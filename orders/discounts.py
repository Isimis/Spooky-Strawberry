...
class DiscountResult:
    ...
    shipping_discount_total: Decimal = Decimal("0.00")
    error: str = ""
...
def evaluate_discount(value, *, subtotal, shipping_cost=Decimal("0.00"), user=None, email=""):
    ...
    shipping_cost = Decimal(shipping_cost or "0.00").quantize(MONEY_PLACES)
    ...
    if discount_code.applies_to == DiscountCode.APPLIES_TO_SHIPPING:
        discount_base = shipping_cost
    elif discount_code.applies_to == DiscountCode.APPLIES_TO_ORDER:
        discount_base = subtotal + shipping_cost
    else:
        discount_base = subtotal

    if discount_code.discount_type == DiscountCode.TYPE_PERCENT:
        discount_total = (discount_base * discount_code.value / Decimal("100")).quantize(
            MONEY_PLACES, rounding=ROUND_HALF_UP
        )
    else:
        discount_total = Decimal(discount_code.value).quantize(MONEY_PLACES)

    discount_total = min(discount_total, discount_base)
    if discount_code.applies_to == DiscountCode.APPLIES_TO_SHIPPING:
        product_discount_total = Decimal("0.00")
        shipping_discount_total = discount_total
    elif discount_code.applies_to == DiscountCode.APPLIES_TO_ORDER:
        # Rabat na całe zamówienie rozdzielamy proporcjonalnie między produkty i dostawę.
        # Dzięki temu kod procentowy faktycznie obniża oba elementy podsumowania.
        if discount_base:
            product_discount_total = (discount_total * subtotal / discount_base).quantize(
                MONEY_PLACES, rounding=ROUND_HALF_UP
            )
        else:
            product_discount_total = Decimal("0.00")
        shipping_discount_total = discount_total - product_discount_total
    else:
        product_discount_total = discount_total
